...
const state = {
  ...
  // Custom Settings (Saved to LocalStorage + Supabase)
  ...
  lastKnownLat: null,
  lastKnownLng: null,
  activeSOSAlertId: null,    // Supabase UUID of the currently active SOS alert
  sosLocationInterval: null  // location ping interval during SOS
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize Supabase (anonymous auth + cloud sync) — non-blocking
  if (typeof initSupabase === 'function') {
    initSupabase().then(() => {
      // After Supabase is ready, sync contacts from cloud
...
function prefetchLocation() {
  ...
  // Load contacts: localStorage first (instant), then Supabase sync in background
  function loadContacts() {
    ...
    if (localData) {
      ...
      state.contacts = [
        { id: 1, name: "Police Patrol Hub", phone: "112", sbId: null },
        { id: 2, name: "Women Cell Emergency", phone: "1091", sbId: null }
      ];
      saveContactsToLocal();
    }
    renderContactsList();
  ...
  // Pull contacts from Supabase and merge with local (cloud wins)
  async function syncContactsFromCloud() {
    const cloudContacts = await sbFetchContacts();
    if (!cloudContacts) return; // Supabase offline

    if (cloudContacts.length > 0) {
      // Map Supabase rows to local format
      state.contacts = cloudContacts.map(c => ({
        id: c.id,   // use Supabase UUID as id
        sbId: c.id,
        name: c.name,
        phone: c.phone
      }));
      saveContactsToLocal();
      renderContactsList();
      console.log('[Supabase] Contacts synced from cloud:', state.contacts.length);
    } else if (state.contacts.length > 0) {
      // Cloud is empty but local has data — push local contacts to cloud
      for (const c of state.contacts) {
        const created = await sbAddContact(c.name, c.phone);
        if (created) c.sbId = created.id;
      }
      saveContactsToLocal();
      console.log('[Supabase] Local contacts pushed to cloud.');
    }
  }
  ...
  document.getElementById("contact-form").addEventListener("submit", async (e) => {
    ...
    // Save to Supabase first (get UUID back)
    const cloudRow = await sbAddContact(newContact.name, newContact.phone);
    if (cloudRow) {
      newContact.id = cloudRow.id;  // use Supabase UUID
      newContact.sbId = cloudRow.id;
    }
  ...
  async function deleteContact(id) {
    ...
    if (contact && contact.sbId) {
      await sbDeleteContact(contact.sbId);
    }
  ...
  function triggerSOSAlert() {
    ...
    // Log SOS to Supabase (with current location if available)
    const lat = state.lastKnownLat || null;
    const lng = state.lastKnownLng || null;
    sbLogSOS(lat, lng);
  ...
  function deactivateSOSAlert() {
    ...
    // Mark SOS as deactivated in Supabase
    sbDeactivateSOS();
      ...
      const uploadInterval = setInterval(() => {
        ...
        if (progress === 100) {
          ...
          setTimeout(() => {
            ...
            // Log recording metadata to Supabase
            sbLogRecording(`Evidence_Sec_${state.recordingsLog.length}.wav`, state.recordingTime);
